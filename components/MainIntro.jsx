            ...
            <div className={`relative ${props.sectionPadding}`}>
                <div className="container">
                    ...
                    <div className="flex items-center justify-between flex-wrap cxl:flex-nowrap gap-[24px] px-[10px] xs:px-0">
                        {introData.introData.map((item, index) => (
                            <div key={index} className={`main-intro-item-wrapper w-full clg:w-[48.612%] cxl:w-1/3 relative border-solid border-[#e0e0e0] border-[1px] p-[30px] xs:p-[40px] sm:p-[48px]  rounded-[12px] transition-all duration-300 hover:shadow-custom-main-intro hover:border-[#ffffff] dark:hover:border-[#2f2f2f] dark:border-[#2f2f2f] ${props.cardWrapper}`}>
                                <span className={`mb-2 sm:mb-3 flex ${props.cardIconColor}`}>
                                    <IconComponent icon={item.icon} />
                                </span>
                                <h3 className="text-[#222222] text-[24px] sm:text-[28px] mb-3 sm:mb-[18px] font-bold font-poppins dark:text-white">{item.title}</h3>
                                <p className="text-[16px] sm:text-[18px] font-opensans text-[#615978] dark:text-main-dark-two">{item.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
                <div className={`hidden ${props.introShape}`}>
