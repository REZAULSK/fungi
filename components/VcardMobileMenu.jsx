...
export default function VcardMobileMenu() {
  ...
  return (
    <div className="">
      <button
        onClick={toggleMenu}
        className="p-2 text-white rounded-[3px] focus:outline-none border-[1px] border-[#ffffff] border-opacity-100"
      >
        {isOpen ? (
          <svg
            width="20" 
      ...
      {isOpen && (
        <div className="absolute top-[3rem] bg-white dark:bg-[#282930] shadow-lg w-[97%] left-1/2 -translate-x-1/2">
          ...
          {menuItems.map((section, index) => (
                <span key={section.title} className="nav-item px-5 py-3 border-b border-[#e0e0e0] w-full">
                    <ScrollLink
                        ...
                        smooth={true}
                        duration={500}
                        className={`nav-link text-[18px] font-bold cursor-pointer flex items-center gap-3 ${activeLink === section.title ? 'text-[#adff00]' : 'text-dark dark:text-white'}`}
                        onSetActive={handleSetActive}
                        activeClass="active"
                    >
                        <span className={`transition-all duration-300 main-service-icon ${activeLink === section.title ? 'text-[#adff00]' : 'text-[#222222] dark:text-white hover:text-[#adff00]'}`}>
                            <svg
                                width="22"
                                height="22"
                                fill="currentColor"
          ...
          <ul className="flex gap-6 items-center py-6 px-5">
                {socialLinks.map((link, index) => (
                    <li key={index} className=''>
                        <a href={link.link} target="_blank" rel="noreferrer" className={`transition-all duration-300 hover:text-[#adff00] ${link.icon === 'FaFacebookF' && 'text-[#1877f2]'} ${link.icon === 'FaTwitter' && 'text-[#1da1f2]'} ${link.icon === 'FaLinkedin' && 'text-[#0077b5]'}  ${link.icon === 'FaInstagram' && 'text-[#405de6]'}`}>
                            {(() => {
                                switch (link.icon) {
                                case 'FaFacebookF':
                                    return <FaFacebookF />
